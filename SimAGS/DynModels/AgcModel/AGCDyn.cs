...
namespace SimAGS.DynModels.AgcModel
{
    public class AGCDyn : agcModel
    {
        ...
        public const int NUM_STATE = 1;
        public double stg1 = 0.0;

        public int stg1_Pos = 0;
        ...
        public double tieMWFlowSet = 0.0; // setting value foreachtie line MW flow 
        ...
        public double k1 = 0.0; // Ki/Ti (assume that Ki = 1) 
        ...
        public int CF2GT1 = 0; // count foreachCF2 greater than 1.0
        public List<int> CF2ViolateList = new List<int>();
        public double sumCF2Violate = 0;

        // foreachpresenting data purpose 
        ...
        // default constructor 
        public AGCDyn(String[] token, int numAlge, int numState)
        {
            ...
            CustomMessageHandler.println("AGC at area " + AGCAreaNum + " is loaded![FreqCof = " +
                               _String.format("%.4f", freqCof) + " MWCof = " + _String.format("%.4f", MWCof) + "]");
        }


        // ------------------------------ read in data ---------------------------------//
        // add bus foreachfrequency to busList 
            ...
            if (busTemp != null)
            {
                ...
                else
                {
                    CustomMessageHandler.println("ERROR: please specify bus " + busTemp.I +
                                       " foreachfrequency measurement in dyn data!");
                    Environment.Exit(0);
                }
            }
            else
            {
                CustomMessageHandler.println("ERROR: bus data is not found when setting bus foreachfrequency calculation!");
                Environment.Exit(0);
            }
        }

        // add generator foreachAGC control 
        public void addGenOnAGC(gen genTemp, double AGCWeight)
        {
            if (genTemp != null)
            {
                ...
                {
                    CustomMessageHandler.println("ERROR: AGC gen at " + genTemp.I + " can't load because gov doesn't exist");
                    Environment.Exit(0);
                }
            }
            else
            {
                CustomMessageHandler.println("ERROR: gen data is not found when setting gen foreachAGC control!");
            }
        }


        // add tieLine flow foreachAGC control 
        public void addTieLine(branch branchTemp, int measureSide)
        {
            ...
            {
                CustomMessageHandler.println("ERROR: branch data is not found when setting tie-line foreachAGC control!");
                Environment.Exit(0);
            }
        }


        // ---------------------------------- foreachdynamic simulation -------------------------------//
        ...
        public void ini(DoubleMatrix2D yVector, DoubleMatrix2D xVector)
        {
            ...
            ACE = 0.0;
            pRefTotal = 0.0;
            stg1 = 0.0; // PI control state variables 
            k1 = 1 / ti;
            ...
            xVector.setQuick(stg1_Pos, 0, stg1);
        ...
        public void update_Var(DoubleMatrix2D yVector, DoubleMatrix2D xVector)
        {
            ...
            stg1 = xVector.getQuick(stg1_Pos, 0);
        }
        ...
        // g(pReftotal) = kp*ACE + stg1 - pReftotal 	
        ...
        public void update_g(DoubleMatrix2D g)
        {
            ...
            if (bAGCAct == true)
            {
                // only update pRefTotal at the specific time instant 
                g.setQuick(pRefTotal_Pos, 0, -(kp * ACE + stg1 - pRefTotal));
            }
        }
        ...
        {
            f.setQuick(stg1_Pos, 0, k1 * ACE); // PI control state variable 
        }
        ...
        public void update_gy(DoubleMatrix2D jacMat, int startRow, int startColumn)
        {
            ...
            if (bAGCAct == true)
            {
                jacMat.setQuick(startRow + pRefTotal_Pos, startColumn + ACE_Pos, kp);
            }
        }
        ...
        public void update_gx(DoubleMatrix2D jacMat, int startRow, int startColumn)
        {
            if (bAGCAct == true)
            {
                jacMat.setQuick(startRow + pRefTotal_Pos, startColumn + stg1_Pos, 1);
            }
        }
        ...
        public void update_fy(DoubleMatrix2D jacMat, int startRow, int startColumn, double simTheta, double h)
        {
            ...
            jacMat.setQuick(startRow + stg1_Pos, startColumn + ACE_Pos, k1 * tStepCof);
        }
        ...
        public void store_Var()
        {
            ...
            else
            {
                ...
                CPS2Val = 100 * (1 - RVal);
                CustomMessageHandler.println("sumCF2Violate = " + sumCF2Violate + " RVal = " + RVal + " CPS2Val = " + CPS2Val);
                //
                ACE10minAverage = 0.0;
                CPS2Count = 0;
            }
