...
namespace SimAGS.DynProcessor
{
    public class DynCase
    {
        ...
        {
            pfCase = PfCase;
            dynEventList = new distEventList(DynCase.this);     // event list
            busList = pfCase.sortBusList;
            branchList = pfCase.branchList;
            itrNum = 0;
        }
        ...
        public void loadCaseFile(File dynDataFile, File AGCDataFile, File windDataFile, File contDataFile, File genSchdDataFile, File loadSchdDataFie) throws simException
        {
    ...
    public void init() throws simException
    {
    ...
    fVector = new SparseDoubleMatrix2D(numState, 1);
		...
		// * bus related element
		for (bus busTemp: busList) busTemp.dynIni(yVector, xVector);		// initialize yVector and xVector 
				
		// * branch related element
		for (branch branTemp: branchList) { 
			if (branTemp.bHasMWFlowMeaseure) branTemp.MWFlowMon.ini(yVector, xVector);
} 
		
		// * AGC related element
		for (agcModel agcTemp: AGCList) {
			agcTemp.deactivate(); 
		...
		System.out.println(">>>>>>>> ... Initialization of dynamic model is completed!"); 
	}
	
	
	// main simulation function 
	public bool sim() throws simException
{
		...
		calcThread = new Thread(new Runnable()
{
    ...
    public void run()
    {
        System.out.println(">>>>>>>> ... Time domain simulation begins!");
        ...
        {                                                       // outside loop for entire simulation duration 	
            ...
            if (bSolved == false && h != hStepMin)
            {
                ...
                yVector = yVector_n;
                bSolved = intStep(xVector_n, fVector_n, h, simTheta, setDyntol);            // results stored in xVector and yVector 				
                System.out.println("========== Time step is reduced to hStepMin for divergence case ============");
            }

            if (bSolved == false)
            {                                                   // diverged after maximum iteration number
                System.out.println("ERROR: <Max iteration number is reached> Simluation terminates at t = " + t);
                break;
            }
            else
            {
                ...
                String strDisp = "---->" + String.format("%8.5f", t) + "\t " +
                        //String.format("%1.5f", busList.get(6).volt) + "\t " + 
                        String.format(" iter = %2d", itrNum) + "\t " +
                        String.format(" hStep = %5.3f", h) + "\t " +
                        //String.format(" PLoad = %5.3f @bus%3d", busList.get(11).dynLoad.getDynPInj(), busList.get(11).I) + "\t" + 
                        //String.format("%10.3f", busList.get(23).getAngle(yVector)) + "\t" + 
                        //String.format("%10.3f", busList.get(23).getVolt(yVector)) + "\t" + 
                        //String.format("%10.3f", yVector.getQuick(branchList.get(9).MWFlow_Pos,0)) + "\t" + 
                        //String.format("%1.5f", findGenAt(1, "U1").getMWSetting()) + "\t " + 
                        //String.format("%10.3f", busList.get(8).dynLoad.getDynPInj()) + "\t " + 
                        //String.format("%10.3f", busList.get(8).dynLoad.getDynQInj()) + "\t " + 
                        //String.format("%10.3f", busList.get(0).busGens.get(0).govDyn.limitTest()) + "\t " + 
                        //String.format("%1.5f", genList.get(3).govDyn.getPRef()) + "\t " + 
                        //String.format("%1.2f", genList.get(1).excDyn.getVr()) + "\t " + 
                        //String.format("%1.5f", yVector.getQuick(busList.get(4).vmagPos,0)); // + "\t " + 
                        //String.format("%1.5f", genList.get(1).excDyn.getEfd());
                        //String.format("%1.5f", AGCGrpList.get(0).get_ACE()) + " \t" + 
                        //String.format("%1.5f", AGCCtrl.getpRefTotal()) + 
                        "";
        ...
        if (agcTemp.isActivate())
        {
            ...
            dynEventList.addEvent(new actAGC(t + agcTemp.getNextTimeInterval(), agcTemp));
				...
				if (simDone == true) {
					System.out.println("======== Simulaiton is Completed [Time = " + String.format("%1.3f", (double) (System.currentTimeMillis() - startTime)/1000) + " seconds] =========");
				} else {
					// interrupted case
					if (bInterrupt == true) {
						System.out.println("Info: Program is terminated!"); 
					} else {
						System.out.println("### Error: simulation terminates for divergence at " + String.format("%1.3f", t));
...
public bool intStep(DoubleMatrix2D xVector_n, DoubleMatrix2D fVector_n, double h, double simTheta, double tol)
{
    ...
    {                                                           // inner loop for each time instants			
        ...
        //System.out.println("itr = " + i + " mistmatch = " + mismatch); 

        //System.out.println("========== g equ ================");
        //dataProcess.dispMat(dVar.viewPart(0,0,numAlgeVar,1)); 

        //System.out.println("========== f equ ================");
...
public DoubleMatrix2D update_dg()
{
    ...
    {
        //-------------- calculate dynamic model power injection -----------// 
        for (gen genTemp: busTemp.busGens)
        {
...
public bus findBusAt(int BusNum)
{
    ...
    System.out.println("ERROR: bus(" + BusNum + ") is not found!");
    return null;
}
    */
    }
